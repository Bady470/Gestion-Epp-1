   ...
   public function enviarTodos()
{
    try {
        ...
        // ⚡ consulta optimizada (quitamos cargas innecesarias)
        $pedidos = Pedido::whereHas('usuario', function ($query) use ($user) {
                $query->where('areas_id', $user->areas_id);
            })
            ->where('estado', 'pendiente')
            ->with(['usuario.area', 'ficha.programa']) // ❌ quitamos 'elementos' si no es crítico
            ->get();

        if ($pedidos->isEmpty()) {
            return back()->with('info', 'No hay pedidos pendientes para enviar');
        }

        $areaNombre = $user->area->nombre ?? 'Área desconocida';
        $instructores = $pedidos->pluck('usuario.nombre_completo')->unique();

        // 📧 correo (único proceso pesado)
        $adminEmail = config('mail.from.address');
        Mail::to($adminEmail)->send(
            new PedidosAgrupadosAdmin($pedidos, $areaNombre, $instructores)
        );

        // ⚡ UPDATE MASIVO (sin foreach)
        Pedido::whereIn('id', $pedidos->pluck('id'))
            ->update(['estado' => 'enviado']);

        // ⚡ NOTIFICACIONES OPTIMIZADAS
        $adminsIds = User::where('roles_id', 1)->pluck('id');
        ...
        foreach ($adminsIds as $adminId) {
            $notificaciones[] = [
                ...
                'mensaje' => "Se enviaron {$pedidos->count()} pedidos del área {$areaNombre}.",
                'usuario_accion_id' => $user->id,
                'leida' => false,
        ...
        return back()->with('success', 'Se enviaron ' . $pedidos->count() . ' pedidos correctamente');

    } catch (\Exception $e) {
        \Log::error('Error al enviar pedidos: ' . $e->getMessage());

        return back()->with('error', 'Error al enviar los pedidos');
    }
}
