...
@section('title', 'Gestión de Usuarios - SENA')
...
<div class="container py-4">
    ...
    <div class="header-section d-flex justify-content-between align-items-center">
        <div>
            ...
            <p class="mb-0 opacity-90">Administra los usuarios y sus roles en el sistema</p>
        </div>
        <a href="{{ route('usuarios.create') }}" class="btn btn-light btn-nuevo shadow-sm d-flex align-items-center gap-2 px-4 py-2 rounded-pill fw-bold text-success">
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr class="table-row-hover">
                            <td data-label="ID" class="text-center fw-bold text-primary">#{{ $usuario->id }}</td>

                            <td data-label="Nombre" class="fw-bold text-dark">
                                {{ $usuario->nombre_completo }}
                            </td>

                            <td data-label="Email" class="text-primary">
                                <i class="bi bi-envelope-at me-1 opacity-75"></i> {{ $usuario->email }}
                            </td>

                            <td data-label="Rol">
                                <span class="badge rounded-pill px-3 py-2" style="background: rgba(64, 100, 121, 0.1); color: var(--sena-blue);">
                                    <i class="bi bi-shield-lock me-1"></i> {{ $usuario->rol?->nombre ?? 'Sin rol' }}
                                </span>
                            </td>
                            ...
                            <td data-label="Área">
                                @if($usuario->area)
                                    <span class="text-secondary fw-semibold">
                                        <i class="bi bi-building me-1"></i> {{ $usuario->area->nombre }}
                                    </span>
                                @else
                        ...
                        <tr>
                            <td colspan="6" class="text-center py-5">
                                <div class="py-4">
                                    ...
                                    <h5 class="mt-3 fw-bold text-muted">No hay usuarios registrados</h5>
                                    <p class="text-muted">Comienza creando el primer usuario del sistema.</p>
                                    <a href="{{ route('usuarios.create') }}" class="btn btn-success rounded-pill px-4 mt-2">
